...
- No visible chrome at rest, on every device. Hover and keyboard focus fade the glyph badge in over `--duration-fast` `--ease-default`; the pointer shows `cursor: pointer`. Touch screens get no at-rest affordance by explicit decision (2026-06-10): a coarse-pointer at-rest badge shipped briefly and was reversed the same day; tap-anywhere IS the control and the strips stay chrome-free. Rejected option recorded per contribution rule 10.
