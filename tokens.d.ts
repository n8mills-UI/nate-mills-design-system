/* @n8mills/design-tokens: design tokens for natemills.me.
   GENERATED from tokens.css (built from tokens.json). Do not hand-edit.
   Values are the default (light) theme, resolved to literals. Dark and
   high-contrast theming live in tokens.css. */

export interface DesignTokens {
  "neutral-0": string;
  "neutral-50": string;
  "neutral-100": string;
  "neutral-150": string;
  "neutral-200": string;
  "neutral-300": string;
  "neutral-400": string;
  "neutral-500": string;
  "neutral-550": string;
  "neutral-600": string;
  "neutral-700": string;
  "neutral-800": string;
  "neutral-850": string;
  "neutral-900": string;
  "neutral-950": string;
  "lime-50": string;
  "lime-100": string;
  "lime-200": string;
  "lime-300": string;
  "lime-400": string;
  "lime-500": string;
  "lime-550": string;
  "lime-600": string;
  "lime-700": string;
  "lime-800": string;
  "lime-900": string;
  "lime-950": string;
  "yellow-50": string;
  "yellow-100": string;
  "yellow-200": string;
  "yellow-300": string;
  "yellow-400": string;
  "yellow-500": string;
  "yellow-550": string;
  "yellow-600": string;
  "yellow-700": string;
  "yellow-800": string;
  "yellow-900": string;
  "yellow-950": string;
  "brand-primary": string;
  "brand-primary-rgb": string;
  "brand-primary-dim": string;
  "brand-primary-vivid": string;
  "brand-secondary": string;
  "brand-spot": string;
  "brand-ink": string;
  "brand-ink-rgb": string;
  "brand-on-primary": string;
  "brand-on-ink": string;
  "brand-gradient": string;
  "ext-brand-anthropic-large": string;
  "ext-brand-anthropic-body": string;
  "ext-brand-linkedin": string;
  "ext-brand-bupa-blue": string;
  "ext-brand-bupa-blue-dark": string;
  "ext-brand-bupa-blue-subtle": string;
  "ext-brand-github": string;
  "ext-brand-wcag-amber": string;
  "fx-glitch-magenta": string;
  "fx-glitch-cyan": string;
  "fx-spectrum-1": string;
  "fx-spectrum-2": string;
  "fx-spectrum-3": string;
  "fx-spectrum-4": string;
  "fx-spectrum-5": string;
  "fx-spectrum-6": string;
  "fx-spectrum-spin": string;
  "glitch-burst": string;
  "glitch-shift": string;
  "glitch-strength": string;
  "color-bg": string;
  "color-surface": string;
  "color-surface-sunken": string;
  "color-surface-inverse": string;
  "color-surface-inverse-sunken": string;
  "color-text-primary": string;
  "color-text-secondary": string;
  "color-text-tertiary": string;
  "color-border": string;
  "color-border-strong": string;
  "color-border-brand": string;
  "color-border-hover": string;
  "color-accent": string;
  "color-accent-hover": string;
  "color-accent-subtle": string;
  "color-on-accent": string;
  "color-focus-ring": string;
  "color-link": string;
  "color-link-hover": string;
  "color-on-ink-emphasis": string;
  "color-on-ink-strong": string;
  "color-on-ink-primary": string;
  "color-on-ink-secondary": string;
  "color-on-ink-muted": string;
  "color-on-ink-border": string;
  "color-static-white": string;
  "color-static-white-hover": string;
  "color-accent-emphasis": string;
  "color-dot-inactive": string;
  "color-dot-active": string;
  "color-text-muted-emphasis": string;
  "color-on-ink-hover": string;
  "color-on-primary-strong": string;
  "color-on-primary-muted": string;
  "color-feedback-background-error": string;
  "color-on-feedback-error": string;
  "color-overlay-soft": string;
  "color-overlay-base": string;
  "color-overlay-strong": string;
  "font-display": string;
  "font-body": string;
  "font-mono": string;
  "font-feature-mono": string;
  "size-2xs": string;
  "size-xs": string;
  "size-sm": string;
  "size-base": string;
  "size-lg": string;
  "size-xl": string;
  "size-2xl": string;
  "size-3xl": string;
  "text-h1": string;
  "text-h2": string;
  "text-body": string;
  "text-label": string;
  "text-caption": string;
  "text-meta": string;
  "text-nav": string;
  "display-hero": string;
  "display-section": string;
  "display-card-title": string;
  "display-stat-num": string;
  "display-intro": string;
  "display-tight": string;
  "display-weight": string;
  "measure": string;
  "btn-sm": string;
  "btn-md": string;
  "btn-lg": string;
  "btn-pad-x": string;
  "btn-pad-y": string;
  "btn-gap": string;
  "btn-radius": string;
  "btn-font": string;
  "btn-font-size": string;
  "btn-font-weight": string;
  "btn-glyph": string;
  "btn-glyph-sm": string;
  "btn-glyph-lg": string;
  "btn-transition": string;
  "btn-focus-ring": string;
  "btn-primary-bg": string;
  "btn-primary-fg": string;
  "btn-primary-border": string;
  "btn-primary-bg-hover": string;
  "btn-primary-fg-hover": string;
  "btn-secondary-fg": string;
  "btn-secondary-border": string;
  "btn-icon-border": string;
  "btn-icon-fg": string;
  "header-control": string;
  "header-clearance": string;
  "card-radius": string;
  "card-padding": string;
  "card-border": string;
  "card-bg": string;
  "card-fg": string;
  "card-dark-bg": string;
  "card-dark-fg": string;
  "card-dark-border": string;
  "chip-pad-x": string;
  "chip-pad-y": string;
  "chip-gap": string;
  "chip-icon-size": string;
  "chip-radius": string;
  "chip-font": string;
  "chip-font-size": string;
  "chip-font-weight": string;
  "chip-line": string;
  "chip-fg": string;
  "chip-bg": string;
  "chip-border": string;
  "chip-border-hover": string;
  "badge-pad-x": string;
  "badge-pad-y": string;
  "badge-gap": string;
  "badge-radius": string;
  "badge-font": string;
  "badge-font-size": string;
  "badge-font-weight": string;
  "badge-icon-size": string;
  "badge-dot-size": string;
  "badge-fg": string;
  "badge-border": string;
  "badge-dot": string;
  "dot-radius": string;
  "bar-radius": string;
  "avatar-radius": string;
  "line-display": string;
  "line-heading": string;
  "line-title": string;
  "line-snug": string;
  "line-body": string;
  "weight-regular": string;
  "weight-medium": string;
  "weight-semibold": string;
  "weight-bold": string;
  "weight-extrabold": string;
  "tracking-tight": string;
  "tracking-normal": string;
  "tracking-wide": string;
  "tracking-mono": string;
  "tracking-eyebrow": string;
  "tracking-label": string;
  "space-50": string;
  "space-100": string;
  "space-200": string;
  "space-250": string;
  "space-300": string;
  "space-400": string;
  "space-500": string;
  "space-600": string;
  "space-700": string;
  "space-800": string;
  "space-900": string;
  "space-1000": string;
  "space-1100": string;
  "space-1200": string;
  "section-padding": string;
  "section-head-gap": string;
  "card-pad-compact": string;
  "card-pad-standard": string;
  "card-pad-spacious": string;
  "duration-fast": string;
  "duration-stagger": string;
  "duration-base": string;
  "duration-moderate": string;
  "duration-slow": string;
  "duration-flourish": string;
  "duration-beat": string;
  "duration-expressive": string;
  "duration-marquee": string;
  "ease-default": string;
  "ease-entrance": string;
  "ease-exit": string;
  "ease-emphasized": string;
  "ease-spring": string;
  "ease-linear": string;
  "motion-reveal-rise": string;
  "motion-reveal-shift": string;
  "motion-reveal-scale": string;
  "motion-reveal-slide": string;
  "motion-reveal-begin": string;
  "motion-reveal-settle": string;
  "motion-reveal-stagger": string;
  "radius-none": string;
  "radius-sm": string;
  "radius-md": string;
  "radius-lg": string;
  "radius-xl": string;
  "radius-full": string;
  "border-hairline": string;
  "border-strong": string;
  "border-focus": string;
  "focus-ring-offset": string;
  "container-base": string;
  "container-wide": string;
  "bp-mobile": string;
  "bp-tablet": string;
  "bp-desktop": string;
  "bp-wide": string;
  "bp-ultrawide": string;
  "touch-target-min": string;
  "touch-target-dot": string;
  "logo-tile": string;
  "logo-tile-lg": string;
  "z-base": string;
  "z-dropdown": string;
  "z-sticky": string;
  "z-overlay": string;
  "z-modal": string;
  "z-tooltip": string;
  "z-toast": string;
  "z-skip-link": string;
  "portrait-fade": string;
  "hero-light-warm": string;
  "hero-light-fill": string;
  "alpha-100": string;
  "alpha-200": string;
  "alpha-300": string;
  "alpha-400": string;
  "alpha-500": string;
  "alpha-600": string;
  "alpha-700": string;
  "glass-tint": string;
  "glass-tint-scrolled": string;
  "glass-tint-strong": string;
  "glass-edge-top": string;
  "glass-edge-bottom": string;
  "glass-specular": string;
  "glass-shadow": string;
  "glass-blur": string;
  "glass-blur-webkit": string;
  "glass-blur-frost": string;
  "glass-blur-dim": string;
  "glass-saturate": string;
  "glass-dim": string;
  "glass-dim-scrolled": string;
  "sbtn-pad-y": string;
  "sbtn-pad-x": string;
  "sbtn-gap": string;
  "sbtn-stack-gap": string;
  "sbtn-row-gap": string;
  "sbtn-min-h": string;
  "sbtn-min-h-sm": string;
  "sbtn-radius": string;
  "sbtn-mark": string;
  "sbtn-mark-figma": string;
  "sbtn-mark-sm": string;
  "sbtn-mark-figma-sm": string;
  "sbtn-label-size": string;
  "sbtn-label-weight": string;
  "sbtn-eyebrow-size": string;
  "sbtn-eyebrow-weight": string;
  "sbtn-gh-fg": string;
  "sbtn-li-fg": string;
  "sbtn-radius-sm": string;
  "wcagm-mark": string;
  "wcagm-mark-sm": string;
  "wcagm-pad-y-sm": string;
  "wcagm-pad-x-sm": string;
  "wcagm-stack-gap-sm": string;
  "wcagm-label-size-sm": string;
  "wcagm-eyebrow-size-sm": string;
}

export declare const tokens: DesignTokens;
export default tokens;
