...
Every animation must respect `prefers-reduced-motion`. The reduced-motion override at the bottom of `tokens.css` collapses every duration to 1 ms and disables transforms whenever the OS asks for reduced motion. It is unconditional and native (no on-page motion control). See Section 10, Display preferences.
...
| MotionPauseControl | one pattern: the region itself is an invisible toggle (showreel model) | `assets/sections.css` (`.strip-motion-toggle`), pattern below |
...
THE pause control for every auto-moving region, and the only one: the moving region itself is an invisible toggle, behaving exactly like the showreel videos. Cursor change on hover, click (or Space/Enter) stops, the same again starts. Nothing is ever shown visually: no floating corner button, no glyph, no hover-pause. One interaction model across the whole page (decided 2026-06-10 after two visible variants, a floating corner button and a hover glyph badge, both shipped and were both rejected; recorded per contribution rule 10). Current regions: the hero skill ticker (its region is the whole hero, so the same toggle stops the Spline scene), each logo marquee, the selected-work band, and the xpv2 ring.

**Anatomy.** A full-bleed, fully transparent overlay `<button type="button" class="strip-motion-toggle" data-region-motion aria-pressed="false" aria-label="Pause animation">` (the selected-work band uses `data-sw-motion` instead of `data-region-motion`) sits inside the region container, which must be `position: relative` (free via `[data-motion-region]`). The button is empty. The overlay approach, rather than wrapping the region in a button, keeps real button semantics without feeding the region's text into the accessible name or nesting flow content inside a button. If the region's children are containers with their own semantics (the band's `role="list"`), the overlay sits as a SIBLING above them, never inside.

**Behaviour.**

- Click anywhere on the region toggles pause and resume; Space and Enter do the same from the keyboard (native button activation). The only hover affordance is `cursor: pointer`.
- The accessible name is the stable "Pause animation"; `aria-pressed` alone carries the state (toggle-button pattern, WCAG 4.1.2). The enclosing `role="group"` or section label tells screen-reader users which region the control belongs to.
- No hover-pause. The region runs until clicked, exactly like the videos; a hover-pause variant shipped and was removed for the one-model rule, and a `:focus-within` pause must never return (the toggle keeps focus after a click, so a focus rule holds a resumed region frozen).
- Focus ring (keyboard only): `--color-focus-ring` at 2px, INSET (`outline-offset: -2px`) because the strip containers clip and edge-fade their contents (`overflow: hidden` + `mask-image`), which would swallow the house outside offset. On always-dark surfaces (the hero band) the ring re-scopes to `--brand-lime`, the sanctioned focus colour on dark.
- Pause state lives on the region as `data-motion-paused="1"` (set by the shared handler in `index.html`), driving `animation-play-state` rules; videos, Lottie tiles and the Spline scene inside the region pause through the same handler. A region can be one strip (each logo marquee pauses independently) or a whole section (the hero; the selected-work band).
...
- Known trade-off, accepted: the overlay intercepts the pointer, so per-child hover effects inside a converted region die (the selected-work tiles lost their hover zoom when the band converted). Do not re-add per-child hover styles under an overlay; they can never fire.
...
### Display preferences: native, no panel (W14)

There is **no on-page preferences panel or toolbar.** The site honours the user's own device and browser settings directly, with no UI to operate, which is the GOV.UK / GDS stance: respect the settings the platform already provides instead of bolting a widget on top. This is the considered replacement for the removed UserWay overlay (overlays cannot fix source markup, test poorly with real assistive tech, and carry a credibility and legal liability). A first-party panel (contrast + motion + a browser-zoom pointer) was built and shipped, then removed: it had shrunk to two controls that only duplicated the device's own settings, and its earlier text-size control kept breaking layout. Respecting native settings is the stronger, lower-maintenance signal.

**What the site honours natively (no UI):** `prefers-color-scheme` (light / dark, plus the one-tap theme toggle, which is a brand affordance not an a11y control), `prefers-reduced-motion` (all animation collapses to ~instant), `prefers-contrast: more` (the high-contrast theme below), `prefers-reduced-transparency` (the header glass goes solid), and Save-Data / slow connections (the Spline hero is skipped for the still image). All are emitted by the token generator `tokens/build.mjs` into `tokens.css` as `@media` queries, or handled in `sections.css` / the hero loader.

**Text size is the browser's job.** No on-page control: browser zoom (Cmd / Ctrl and +) scales the whole page uniformly and reflows everything, satisfying 1.4.4 Resize Text (AA) and 1.4.10 Reflow (AA) on its own. Two on-page approaches were tried and rejected, both documented in `docs/plans/`: token scaling could never be uniform (this codebase mixes tokens with intentional hardcoded px, e.g. the testimonial half-pixel sizes), and CSS `zoom` broke this page's JS-driven carousels and scroll coordinates.

**High contrast** is a maximal-contrast theme (~20:1) triggered natively by `@media (prefers-contrast: more)` (macOS "Increase contrast", Windows). It layers over light AND dark (dark wins by later source order plus specificity in both explicit `[data-theme="dark"]` and OS dark). All values resolve to existing primitives via `var()`. Supports 1.4.6 Contrast Enhanced (AAA), 1.4.11 Non-text Contrast (AA). This is the one deliberate, documented extension of the "light and dark only" rule (Section 14): a contrast *modifier* over both themes, not a third brand theme.

**Motion** is the native `@media (prefers-reduced-motion: reduce)` freeze in `tokens.css`: every duration collapses to 1 ms and smooth scroll is disabled when the OS asks. Unconditional, no override (the earlier `data-motion` override layer went with the panel).
